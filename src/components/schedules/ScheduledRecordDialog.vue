...
import { durationLabel, executionTimeLabel, recordStatus, timingNote } from "../../domain/scheduled-task-ui";
...
const statusType = computed(() => {
  if (!run.value) return "default";
  if (run.value.attention === "needs_attention") return "warning";
  if (run.value.run.native_outcome === "completed") return "success";
  if (run.value.run.native_outcome === "failed") return "error";
  return run.value.run.delivery_state === "uncertain" ? "warning" : "default";
});
  ...
  <NModal :show="!!detail" @update:show="show => { if (!show) emit('close'); }" @after-leave="emit('closed')">
    <NCard class="scheduled-record-dialog" title="执行记录详情" closable role="dialog" aria-modal="true" aria-label="执行记录详情" @close="emit('close')">
      <dl v-if="detail" class="scheduled-record-dialog__fields">
        ...
        <dt>执行状态</dt><dd><NTag size="small" round :bordered="false" :type="statusType">{{ recordStatus(detail.record) }}</NTag></dd>
        ...
        <dt>执行耗时</dt><dd :title="timingNote(detail.record.timing)">{{ durationLabel(detail.record.timing) }}</dd>
      </dl>
      ...
      <template #footer>
        <div class="scheduled-record-dialog__actions">
          <NButton :disabled="!run || rerunDisabled" @click="emit('rerun')"><template #icon><YjIcon name="run" size="sm" /></template>重新执行</NButton>
          <NButton type="primary" :disabled="conversationUnavailable" @click="emit('conversation')"><template #icon><YjIcon name="message" size="sm" /></template>查看完整对话</NButton>
        </div>
      </template>
    </NCard>
  </NModal>
