...
const plan = { plan_id: crypto.randomUUID(), revision: 1, schedule_epoch: 1, definition, state: "paused" as const, target_state: "unbound" as const, effective_from: 1790000000, rule_version: 1, tzdb_version: "2026b" };
...
function harness() {
  ...
  const client = createScheduledTaskNativeClient(async (command, args) => {
    ...
    switch (command) {
      ...
      case "schedule_confirm_draft_v1": if (uncertain) throw {schemaVersion:1,requestId:request.requestId,code:"operation_unknown"}; data=plan; output={source_id:receipt.source_id,status:"confirmed",plan_id:plan.plan_id}; break;
...
it("uses a new turn request for clarification and confirms only the reviewed source without grants or regular save",async()=>{
  ...
  expect(h.state.saved.value?.state).toBe("paused");expect(h.calls.some(c=>/save_plan|confirm_grant|confirm_enable|manual_run|confirm_rerun/.test(c.command))).toBe(false);
});
...
it("queries the confirmed source after an uncertain save and never uses management receipts",async()=>{
  ...
  expect(h.calls.filter(c=>c.command==="schedule_confirm_draft_v1")).toHaveLength(1);
