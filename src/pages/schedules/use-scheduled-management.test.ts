...
const plan = { plan_id: crypto.randomUUID(), revision: 1, schedule_epoch: 1, definition, state: "enabled" as const, target_state: "unbound" as const, effective_from: 1790000000, next_at: 1790038800, rule_version: 1, tzdb_version: "2026b" };
const summary = { plan_id: plan.plan_id, name: definition.name, revision: 1, raw_state: "enabled" as const, effective_state: "enabled" as const, target_mode: "dedicated_chat" as const, target_state: "unbound" as const };
const cleanup: (() => void)[] = [];
...
function harness() {
  ...
  let uncertain = false; let observed = true; let deleted = false;
  const native = createScheduledTaskNativeClient(async (command, args) => {
    ...
    switch (command) {
      ...
      case "schedule_save_active_plan_v1": if (uncertain) throw { schemaVersion: 1, requestId: request.requestId, code: "operation_unknown" }; data = plan; break;
      case "schedule_delete_plan_v1": deleted = true; if (uncertain) throw { schemaVersion: 1, requestId: request.requestId, code: "operation_unknown" }; data = { ...plan, state: "deleted" }; break;
      case "schedule_read_plan_mutation_receipt_v1": data = observed ? {observation:"observed", plan_id:plan.plan_id, current_plan: deleted ? {plan:{...plan,state:"deleted"},summary:{...summary,raw_state:"deleted",effective_state:"deleted"}} : {plan,summary}} : {observation:"not_observed"}; break;
...
it("saves and enables in one native write without a separate authorization", async () => {
...
it("preserves an uncertain action across same-scope expiry and queries the original ID without rewriting", async () => {
  ...
  const writes=h.calls.filter(c=>c.command==="schedule_save_active_plan_v1"); expect(writes).toHaveLength(1);
...
it("absence is unknown; only explicit retry resends the exact original management request", async () => {
  ...
  expect(h.calls.filter(c=>c.command==="schedule_save_active_plan_v1")).toHaveLength(1);
  h.uncertainty(false); await h.state.retryMutation();
  const writes=h.calls.filter(c=>c.command==="schedule_save_active_plan_v1"); expect(writes).toHaveLength(2);
...
it("deletes without retaining a success banner or fetching a deleted plan for display", async () => {
  const h = harness(); await vi.waitFor(() => expect(h.state.canManage.value).toBe(true));
  expect(await h.state.mutate("delete", { plan_id: plan.plan_id, expected_revision: plan.revision })).toBe(true);
  expect(h.state.pending.value).toBeNull();
  expect(h.state.notice.value).toBe("");
  expect(h.state.receipt.value).toBeNull();
  expect(h.calls.filter(c => c.command === "schedule_get_plan_v1")).toHaveLength(0);
  expect(h.calls.filter(c => c.command === "schedule_list_plan_cards_v1")).toHaveLength(2);
});
it("clears a confirmed deletion without exposing its retained history receipt", async () => {
  const h = harness(); await vi.waitFor(() => expect(h.state.canManage.value).toBe(true)); h.uncertainty(true);
  await h.state.mutate("delete", { plan_id: plan.plan_id, expected_revision: plan.revision });
  expect(h.state.pending.value?.kind).toBe("delete");
  await h.state.queryReceipt();
  expect(h.state.pending.value).toBeNull();
  expect(h.state.notice.value).toBe("");
  expect(h.state.receipt.value).toBeNull();
  expect(h.calls.filter(c => c.command === "schedule_delete_plan_v1")).toHaveLength(1);
});
it("keeps an explicitly retried deletion quiet and reuses the original request", async () => {
  const h = harness(); await vi.waitFor(() => expect(h.state.canManage.value).toBe(true)); h.uncertainty(true);
  await h.state.mutate("delete", { plan_id: plan.plan_id, expected_revision: plan.revision });
  h.uncertainty(false);
  expect(await h.state.retryMutation()).toBe(true);
  const writes = h.calls.filter(c => c.command === "schedule_delete_plan_v1");
  expect(writes).toHaveLength(2); expect(writes[1]!.request).toEqual(writes[0]!.request);
  expect(h.state.notice.value).toBe(""); expect(h.state.receipt.value).toBeNull();
});
