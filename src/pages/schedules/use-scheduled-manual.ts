...
export function useScheduledManual(m: ReturnType<typeof useScheduledManagement>, prepare: () => Promise<boolean>) {
  ...
  function reset() { epoch++; review.value = null; rerun.value = null; intent.value = null; result.value = null; notice.value = ""; error.value = null; clearTimeout(timer); }
  ...
  async function open(planId: string) {
    ...
    rerun.value = null;
    busy.value = true; error.value = null; notice.value = "正在准备本地执行服务…";
  ...
  async function openRerun(originalRunId: string) {
    if (busy.value || pending.value || !canPrepare.value) return;
    const current = ++epoch; const scope = permissions.selectedTenantId;
    busy.value = true; error.value = null; notice.value = ""; rerun.value = null;
    try {
  ...
  async function accepted(i: Intent, run: RunView) {
    if (!same(i)) return;
    intent.value = null; review.value = null;
    m.receipt.value = null; m.notice.value = "";
    notice.value = "已查到本次运行回执。任务是否结束以下方原生记录为准；本次单次授权未更改原计划和自动额度。";
  ...
  function failed(i: Intent, e: unknown) {
    ...
    if (!["operation_unknown", "context_invalid", "storage_unavailable", "protocol_mismatch"].includes(error.value)) {
      intent.value = null; review.value = null;
      notice.value = "本次提交未获受理，请按错误说明处理后重新审阅。";
  ...
  return { review, rerun, targetTitle, intent, pending, busy, error, notice, result, canPrepare, open, openRerun, confirm, query, retry, refreshResult, closeReview };
}
