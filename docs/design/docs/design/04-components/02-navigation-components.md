...
亮色导航容器为纯白；未选中导航文字、图标使用中性石墨层级。选中项可使用局部青柠软底和青柠强调条，文字保持可读的石墨；实色青柠选中控件使用 on-brand 前景色。暗色导航容器使用中性石墨背景和对应的 selected token。品牌区引用新的商品包裹 / YJ SVG 亮暗变体，不能保留旧手提袋图标。
