...
fn host_authority_fixture_survives_reducer_sqlcipher_restart_and_delete() {
    ...
    let public_task_id = events[0].task_id;
    repository
        .bind_public_task(create.operation_id, public_task_id, unix_seconds())
        .expect("bind content-free Public task");
    repository
        .reschedule_outbox(create.operation_id, unix_seconds())
        .expect("continue Host stage");
    let create = repository
        .claim_next_conversation_outbox(unix_seconds(), 30)
        .expect("claim bound create outbox")
        .expect("bound create outbox exists");
    repository
    ...
    for (index, mut event) in events.into_iter().enumerate() {
        // The fixture owns Host/Runtime identities. Once the content-free Public Task is bound,
        // Host must echo that authority identity rather than the Desktop-local session UUID.
        event.task_id = public_task_id;
        match reducer
